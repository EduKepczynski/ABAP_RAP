...
CLASS ltcl_integration_test IMPLEMENTATION.
  ...
  METHOD create_travel.
    ...
    DATA travels_in TYPE TABLE FOR CREATE zekf_dd_leg_travel\\travel.
    ...
    MODIFY ENTITIES OF zekf_dd_leg_travel
        ENTITY travel
    ...
    SELECT * FROM zekf_dd_leg_travel WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
  ...
  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities = VALUE #( ( i_for_entity = 'zekf_dd_leg_travel' )
                                  ( i_for_entity = 'zekf_dd_leg_booking' ) )
                                ).
  ENDMETHOD.
